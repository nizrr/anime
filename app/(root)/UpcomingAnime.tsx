...
export const UpcomingAnime = ({ anime }: { anime: any }) => {
   ...
   return (
      <section className="w-full h-dvh mt-[-80px]">
         <Carousel
            plugins={[
               Autoplay({
                  delay: 10000,
            ...
            <CarouselContent className="-ml-4 ">
               {data?.map((anime: any, index: number) => {
                  const videoId =
                     anime.trailer?.youtube_id || getYoutubeId(anime.trailer?.embed_url)
                        ...
                        <div className="bg-accent h-dvh overflow-hidden relative w-full">
                           ...
                           <div className="absolute left-30 top-1/2 -translate-y-1/2 z-10 space-y-1">
                              <div className="flex gap-2">
                                 <Badge variant="outline">Upcoming Anime</Badge>
                                 <Badge variant="outline">{anime.type}</Badge>
                              </div>
                              <h1 className="text-white font-bold font-clash  text-6xl max-w-3xl">
                                 {anime.title}
                              </h1>
                              <p className="text-white/80 font-clash line-clamp-2 max-w-xl ">
                                 {anime.synopsis}
                              </p>
                              ...
                              <div className="flex gap-2 mt-4">
                                 ...
                                 <Button className="mt-4 group" variant="ghost" size={'lg'}>
                                    Add to List{' '}
                                    <Icon
                                       icon="mdi:plus"
                                       className="group-hover:translate-x-1 transition-all duration-300"
            ...
            <CarouselPrevious className="absolute left-10 top-1/2 -translate-y-1/2 backdrop-blur-xs bg-white/5 border border-white/20 size-15" />
            <CarouselNext className="absolute right-10 top-1/2 -translate-y-1/2 backdrop-blur-xs bg-white/5 border border-white/20 size-15" />
         </Carousel>
         <div className=" mt-4 mx-2 absolute bottom-10 w-full px-10">
            <div className="flex gap-4">
               {data?.map((anime: any, index: number) => (
                  <div
                     key={index}
                     onClick={() => api?.scrollTo(index)}
                     className="w-full h-[200px] rounded-lg relative cursor-pointer">
                     <img
